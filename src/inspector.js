...
class InspectorView {
  ...
  update(id, name, groups, origin) {
    ...
    const serialize = (item) => {
      ...
      const position = origin?.range.start;
      const range = item.location?.range;
      const atOrigin =
        position &&
        range &&
        item.location.uri.toString() === origin.uri.toString() &&
        (position.line > range.start.line ||
          (position.line === range.start.line && position.character >= range.start.character)) &&
        (position.line < range.end.line ||
          (position.line === range.end.line && position.character < range.end.character));

      if (item.inspected || atOrigin) {
...
.tab-icon { color:var(--vscode-charts-yellow); width:16px; height:16px; flex-shrink:0; fill:none; stroke:currentColor; stroke-width:1.2; stroke-linecap:round; stroke-linejoin:round; }
      ...
      function row(item, parent, path) {
        ...
        if (item.kind) {
          const paths = {
            ...
            Effect: ['M14.5 8a6.5 6.5 0 1 1-13 0 6.5 6.5 0 0 1 13 0Z', 'M11 8a3 6.5 0 1 1-6 0 3 6.5 0 0 1 6 0Z', 'M8 1.5v13M1.5 8h13M2.6 4.5h10.8M2.6 11.5h10.8'],
