...
import { BookEvents } from '../store/books.events';
import * as booksState from '../store/books.state';
...
export const bookExistsGuard = (
  ...
  function waitForCollectionToLoad(): Observable<boolean> {
    return store.select(booksState.selectCollectionLoaded).pipe(
  ...
  function hasBookInStore(id: string): Observable<boolean> {
    return store.select(booksState.selectBookEntities).pipe(
  ...
  function hasBookInApi(id: string): Observable<boolean> {
    return googleBooks.retrieveBook(id).pipe(
      map((bookEntity) => BookEvents.loadBook({ book: bookEntity })),
